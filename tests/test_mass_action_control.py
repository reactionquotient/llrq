...
class TestSingleReaction:
    ...
    def setup_method(self):
        ...
        self.network = ReactionNetwork(
            self.species_ids, self.reaction_ids, self.S, 
            self.species_info, self.reaction_info
        )
    ...
    def test_equilibrium_without_control(self):
        ...
        sim = MassActionSimulator(
            self.network, self.rate_constants,
            B=self.solver._B, K_red=self.solver._B.T @ self.solver.dynamics.K @ self.solver._B
        )
        ...
        np.testing.assert_allclose(Q_final[0], expected_Keq, rtol=1e-2,
                                 err_msg="System should reach correct equilibrium")
    ...
    def test_control_mapping_consistency(self):
        ...
        np.testing.assert_allclose(delta_computed, delta_expected[0], rtol=1e-10,
                                 err_msg="Control mapping should be mathematically consistent")
    ...
    def test_steady_state_control(self):
        ...
        np.testing.assert_allclose(Q_final[0], expected_Q, rtol=5e-2,
                                 err_msg="Constant control should drive to predicted equilibrium")
...
class TestThreeCycleValidation:
    ...
    def test_equilibrium_consistency(self):
        ...
        np.testing.assert_allclose(Q_final, expected_Keq, rtol=1e-2,
                                 err_msg="Mass action equilibrium should match rate constant ratios")
