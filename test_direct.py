...
def test_two_reactions():
    ...
    result = network.compute_dynamics_matrix(
        equilibrium_point=[1.0, 1.5, 0.5],
        forward_rates=[2.0, 1.0],
        backward_rates=[1.0, 2.0],
        mode='equilibrium'
    )
...
def test_algorithm_components():
    ...
    assert np.isclose(phi_forward, phi_backward, rtol=0.1), "Forward and backward fluxes should be approximately equal at equilibrium"
