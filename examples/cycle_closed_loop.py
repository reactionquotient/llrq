...
def build_and_run(out_dir: str = "llrq_report_cycle"):
    ...
    for i, t in enumerate(t_eval):
        ...
        u_full = G @ np.clip(u_steady - feedback_gain * np.linalg.pinv(Bred) @ error, cfg.uhat_bounds[0], cfg.uhat_bounds[1])
        u_red = B.T @ u_full
    ...
    if Cons is not None and Cons.shape[1] > 0:
        ...
        plt.xlabel("time")
        plt.ylabel("conserved totals")
        plt.title("Conservation (constant up to tolerance)")
    ...
    if Ctraj is not None:
        ...
        try:
            ...
            for k, sid in enumerate(network.species_ids):
                plt.axhline(c_ref[k], color=colors[k], linestyle="--", alpha=0.7, label=f"[{sid}]* (target)")
        except:
    ...
    plt.xlabel("ω (rad/s)")
    plt.ylabel("||H(iω)||₂ (dB)")
    plt.title("Frequency response of A=-K_red")
