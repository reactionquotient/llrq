...
def demonstrate_constant_control():
    ...
    for i, (species, color) in enumerate(zip(network.species_ids, colors)):
        ax.plot(t_uncontrolled, result_uncontrolled['concentrations'][:, i], 
                '--', color=color, alpha=0.7, label=f'{species} (natural)')
        ax.plot(t_controlled, result_controlled['concentrations'][:, i], 
                '-', color=color, linewidth=2, label=f'{species} (controlled)')
    ...
    for i, rxn in enumerate(network.reaction_ids):
        ax.plot(t_controlled, result_controlled['reaction_quotients'][:, i], 
                linewidth=2, label=f'Q_{rxn}')
    ...
    ax = axes[1, 0]
    ax.plot(t_uncontrolled, result_uncontrolled['concentrations'][:, -1], 
            '--', linewidth=3, label='Natural', color='red', alpha=0.7)
    ax.plot(t_controlled, result_controlled['concentrations'][:, -1], 
            '-', linewidth=3, label='Controlled', color='green')
    ...
    ax = axes[1, 1]
    ax.plot(t_controlled, result_controlled['concentrations'][:, -1], 
            '-', linewidth=2, label='No disturbance', color='green')
    ax.plot(t_disturbed, result_disturbed['concentrations'][:, -1], 
            '-', linewidth=2, label='With disturbance', color='blue')
    ax.axhline(y=target_concentrations[-1], color='green', linestyle=':', linewidth=2, label='Target')
    ...
    target_error_controlled = np.linalg.norm(
        result_controlled['concentrations'] - target_concentrations, axis=1)
    target_error_natural = np.linalg.norm(
        result_uncontrolled['concentrations'] - target_concentrations, axis=1)
    
    ax.semilogy(t_controlled, target_error_controlled, 
                '-', linewidth=2, label='Controlled', color='green')
    ax.semilogy(t_uncontrolled, target_error_natural, 
                '--', linewidth=2, label='Natural', color='red', alpha=0.7)
    ...
    plt.suptitle('LLRQ Constant Control: Linear Algebra Solves Complex Control Problems', 
