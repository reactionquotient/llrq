...
def simulate_linear_dynamics(network, controller, cfg, t_eval):
    ...
    c0 = controller.solver._parse_initial_dict(
        {sid: info.get("initial_concentration", 0.0) for sid, info in network.species_info.items()}
    )
    ...
    for i in range(n):
        C_traj[i] = controller.solver._compute_concentrations_from_reduced(Q_traj[i : i + 1], c0, True)

    return {"time": t_eval, "y": Y, "u": U, "Q": Q_traj, "C": C_traj, "method": "Linear LLRQ"}
...
def simulate_mass_action_dynamics(network, controller, cfg, t_eval):
    ...
    rate_constants_ma = {"R1": (3.0, 1.5), "R2": (1.0, 2.0), "R3": (3.0, 3.0)}  # Keq = 2.0  # Keq = 0.5  # Keq = 1.0
    ...
    result = sim.simulate(t_eval, control_function, disturbance_function=disturbance_function)
...
def build_and_run_comparison(out_dir: str = "llrq_linear_vs_mass_action"):
    ...
    rate_constants = {"R1": (3.0, 1.5), "R2": (1.0, 2.0), "R3": (3.0, 3.0)}  # Keq = 2.0  # Keq = 0.5  # Keq = 1.0
    ...
    print(
        f"  Initial individual: [{linear_result['C'][0][0]:.6f}, {linear_result['C'][0][1]:.6f}, {linear_result['C'][0][2]:.6f}]"
    )
    print(f"  Final concentrations: {linear_result['C'][-1]}")
    print(
        f"  Final individual: [{linear_result['C'][-1][0]:.6f}, {linear_result['C'][-1][1]:.6f}, {linear_result['C'][-1][2]:.6f}]"
    )
    ...
    for i, (species, color) in enumerate(zip(species_names, colors)):
        ax = axes[i]
        ax.plot(t_eval, linear_result["C"][:, i], "-", color=color, linewidth=2, label=f"Linear LLRQ")
        if mass_action_result:
            ax.plot(t_eval, mass_action_result["concentrations"][:, i], "--", color=color, linewidth=2, label=f"Mass Action")
