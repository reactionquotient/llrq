...
dynamics = LLRQDynamics(network, 
                       equilibrium_constants=np.array([2.0, 1.5, 3.0]),
                       relaxation_matrix=np.eye(3))
...
ctrl = LQRController(solver, controlled_reactions=['R1', 'R3'],
                    Q=1.0*np.eye(solver._rankS), R=0.1*np.eye(2),
...
res = solver.simulate_closed_loop(
    initial_conditions=init_conc_dict,
    t_span=(0.0, 50.0),
    controller=ctrl,
    y_ref=y_ref,
    n_points=1500
