...
def compute_concentrations_from_quotients(
    Q_t: np.ndarray, c0: np.ndarray, network, B: np.ndarray, lnKeq_consistent: np.ndarray, enforce_conservation: bool = True
) -> Optional[np.ndarray]:
...
def apply_state_disturbance_to_concentrations(
    c_current: np.ndarray, d_reduced: np.ndarray, network, B: np.ndarray, lnKeq_consistent: np.ndarray
) -> np.ndarray:
    ...
    c_new = compute_concentrations_from_quotients(Q_new, c_current, network, B, lnKeq_consistent, enforce_conservation=True)
